...
class Hero:
    def __init__(self, start_room, save):
    ...
    def count_stats(self):
        eq_armor = armor[self.save.armor]
        self.defence = eq_armor.defence
        self.cover = eq_armor.cover

        eq_weapon = weapons[self.save.weapon]
        self.attack = eq_weapon.dmg

        skills = self.save.skills
        if skills[1] == 't':
            self.attack += 1
        if skills[2] == 't':
            self.fight_speed += 5
...
class Monster:
    def __init__(self, run_num):
        self.lvl = run_num
        self.hp = 20 + self.lvl * 5
        self.dmg = self.lvl * 10

        self.attack_rail = None

    def attack(self):
        self.attack_rail = randint(0, 2)
